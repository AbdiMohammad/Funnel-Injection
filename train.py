...
def get_model(args):
    MODEL_DICT = {
        ...
        'resnet20': cifar_resnet_tiny.resnet20,
        'resnet32': cifar_resnet_tiny.resnet32,
        'resnet44': cifar_resnet_tiny.resnet44,
        'resnet56': cifar_resnet_tiny.resnet56,
        'resnet110': cifar_resnet_tiny.resnet110,
    }

    return MODEL_DICT[args.model](num_classes=args.num_classes)
...
def load_data(args):
    ...
    if args.dataset == 'cifar10':
        ...
        train_ds = datasets.CIFAR10(args.data_path, train=True, download=True, transform=train_transform)
        val_ds = datasets.CIFAR10(args.data_path, train=False, download=False, transform=val_transform)
        ...
        train_ds = datasets.CIFAR100(args.data_path, train=True, download=True, transform=train_transform)
        val_ds = datasets.CIFAR100(args.data_path, train=False, download=True, transform=val_transform)
        args.input_size = (1, 3, 32, 32)
    ...
    val_loader = torch.utils.data.DataLoader(
        val_ds, batch_size=args.batch_size, sampler=val_sampler, num_workers=args.workers, pin_memory=True
    )
...
def main(args):
    ...
    if opt_name.startswith("sgd"):
        ...
        optimizer = torch.optim.RMSprop(
            model.parameters(), lr=args.lr, momentum=args.momentum, eps=0.0316, alpha=0.9
        )
    elif opt_name == "adamw":
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument("--output-dir", default="run", type=str, help="path to save outputs")
